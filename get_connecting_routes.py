...
def get_connecting_routes(route_graphs: List[nx.MultiDiGraph]):
    ...
    for toll_node in toll_graph.nodes:
        for route_graph in route_graphs[1:]:
            x, y = toll_graph.nodes[toll_node]['x'], toll_graph.nodes[toll_node]['y']
            nearest_node = ox.distance.nearest_nodes(route_graph, X=x, Y=y, return_dist=False)
...
def get_traffic_aware_durations(route_graphs: List[nx.MultiDiGraph], connections, origin, destination):
